...
void HdfsOrcScanner::OrcMemPool::ThrowMemLimitExceeded() {
  ResourceError::ThrowAndSetParseStatus(scanner_, "memory limit exceeded");
}
...
char* HdfsOrcScanner::OrcMemPool::malloc(uint64_t size) {
  if (!mem_tracker_->TryConsume(size)) ThrowMemLimitExceeded();
  char* addr = static_cast<char*>(std::malloc(size));
  if (addr == nullptr) {
    mem_tracker_->Release(size);
    ThrowMemLimitExceeded();
  }
...
void HdfsOrcScanner::ScanRangeInputStream::read(void* buf, uint64_t length,
  ...
  if (!status.ok()) {
    string msg = Substitute("Stop reading $0 (offset=$1, length=$2): $3",
        filename_, offset, length, status.GetDetail());
    VLOG_QUERY << msg;
    ResourceError::ThrowAndSetParseStatus(scanner_, msg);
  }
}
...
Status HdfsOrcScanner::ProcessFileTail() {
  ...
  try {
    ...
    return parse_status_;       // We've set the message in parse_status_.
